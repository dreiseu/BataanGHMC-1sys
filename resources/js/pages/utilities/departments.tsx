...
export default function UtilitiesDepartments({ departments }: Props) {
    ...
    function openEditDialog(item: DepartmentRow) {
                ...
                <section className="relative overflow-hidden rounded-3xl border border-[#00D4FF]/20 bg-gradient-to-br from-[#0F172A] via-[#1E293B] to-[#0F172A] p-8 sm:p-10 shadow-lg min-h-[180px] flex flex-col justify-center group">
                    ...
                    <div className="relative z-10 w-full max-w-3xl">
                        ...
                        <p className="mt-3 text-base sm:text-lg text-slate-300 leading-relaxed font-medium max-w-2xl">
                            Configure the departments and wards available in the Location field of IMISS Job Order Requests.
                        </p>
                    </div>
                </section>
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-[#1E293B]/5 text-left text-muted-foreground uppercase tracking-wider text-[11px] font-bold border-b border-muted-foreground/10">
                            <tr>
                                ...
                                <th className="px-5 py-4">Department / Ward</th>
                        ...
                        <tbody>
                            {paginatedDepartments.map((item, index) => (
                                <tr key={`${item.id}-${index}`} className="border-t border-muted-foreground/5 transition-colors hover:bg-muted/30">
                                    <td className="px-5 py-4 font-mono text-xs text-muted-foreground">{item.Code || '—'}</td>
                                    ...
                                    <td className="px-5 py-4 text-center">
                                        <div className="flex justify-center gap-2">
                                            ...
                                            <button
                                                onClick={() => setDepartmentToDelete(item)}
                    ...
                    {totalPages > 1 && (
                        <div className="flex flex-col sm:flex-row items-center justify-between px-5 py-4 border-t border-muted-foreground/10 gap-4">
                            <span className="text-sm text-muted-foreground font-medium">
                                Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, filteredDepartments.length)} of {filteredDepartments.length} records
                            </span>
            ...
            <Dialog open={showAddForm} onOpenChange={(open) => { setShowAddForm(open); if (!open) resetDialog(); }}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>This appears in the "Select a department or ward" list on the Job Order Request form.</DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-2">
                        <div className="grid gap-2">
                            ...
                            <p className="text-[10px] text-muted-foreground">Optional short code. Leave blank if not needed.</p>
                        </div>
                        <div className="grid gap-2">
                            <label className="text-sm font-medium">Department / Ward <span className="text-destructive">*</span></label>
                            <Input
                                value={form.Department}
                                onChange={(e) => setForm({ ...form, Department: e.target.value })}
                                placeholder="e.g. ADMITTING"
                            />
                        ...
                        <Button
                            ...
                            onClick={() => {
                                ...
                                if (editingDepartment) {
                                    router.put(`/utilities/departments/${editingDepartment.id}`, form, {
                                    ...
                                    router.post('/utilities/departments', form, {
            ...
            <AlertDialog open={!!departmentToDelete} onOpenChange={(open) => { if (!open) setDepartmentToDelete(null); }}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will permanently delete <strong>{departmentToDelete?.Department}</strong>. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                            ...
                            onClick={() => {
                                ...
                                setIsDeleting(true);
                                router.delete(`/utilities/departments/${departmentToDelete.id}`, {
                                    onSuccess: () => { toast.success('Deleted successfully.'); setDepartmentToDelete(null); },
